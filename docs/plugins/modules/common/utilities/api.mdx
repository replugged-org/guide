...
{/* [!]: `Backoff`, `convertSkemaError`, `setAwaitOnline`, `setRequestPatch`, `V6OrEarlierAPIError` and `V8APIError` aren't documented */}

### Props

| Name                           | Type                  | Description                                                                  |
| ------------------------------ | --------------------- | ---------------------------------------------------------------------------- |
| `HTTP`                         | [`HTTP`](#HTTP)       | Contains functions for making HTTP requests                                  |
| `INVALID_FORM_BODY_ERROR_CODE` | number                | Error code for invalid form bodies                                           |
...
| Name    | Parameters                                                                                                        | Return Type                     | Description              |
| ------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------- | ------------------------ |
| `del`   | <Required /> `req`: string \| [`HTTPRequest`](#HTTPRequest) <br /> `callback`: (response: `HTTPResponse`) => void | [`HTTPResponse`](#HTTPResponse) | Fetch with DELETE method |
...
| `interceptResponse` | (response: `Response`, retry: (headers: Record\<string,&nbsp;string>, interceptResponse: `interceptResponse`) => void, reject: (reason: Error) => void) => boolean |           | Function ran to intercept the request response; check out [superagent](https://ladjs.github.io/superagent) Response format                            |
